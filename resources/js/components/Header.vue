<template>
  <div>
    <v-navigation-drawer v-model="drawer" app clipped>
          ...
          <template #activator>
            <ProjectDialog @close="isPush = false">
              <template #activator="{ on }">
                <v-icon small v-on="on" @click="pushIcon()">
                  fas fa-plus
                </v-icon>
              </template>
            </ProjectDialog>
          ...
          >
            ...
            >
              ...
              <v-list>
                <ProjectDialog
                  :dialog-title="edit"
                  :dialog-btn-text="editBtn"
                  :edit-data="projectList"
                  @close="editDialogClose, closeProjectMenu(index)"
                >
                  <template #activator="{ on }">
                    <v-list-item small v-on="on">
      ...
      <v-dialog v-model="deleteProjectDialog" persistent max-width="300">
        <v-card>
          ...
          <v-card-text>
            {{ deleteProjectText }}を削除してもよろしいですか？
          </v-card-text>
          <v-card-actions>
            <v-btn
    ...
    <v-app-bar class="indigo" fixed app dark clipped-left>
      ...
      <v-spacer />
      <v-menu bottom right offset-y nudge-width="200">
        <template #activator="{ on, attrs }">
